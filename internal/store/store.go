...
// ErrDuplicateEvent is returned when an event with the same EventID has already been ingested.
...
// IngestTransaction executes event insertion, call upsert, and account stats increment atomically inside a single database transaction.
func (s *Store) IngestTransaction(ctx context.Context, e Event) error {
